...
public class SummaryReport implements IReporter {
	...
	protected static class TestResult {
		...
		public TestResult(ITestContext context) {
			...
			skippedTests = skippedTestDuplicateRemover(passedTests, skippedTests);
			skippedTests = skippedTestDuplicateRemover(failedTests, skippedTests);
			System.out.println("failedTests" +failedTests.size());
			System.out.println("passedTests"+ passedTests.size());
			System.out.println("skippedTests"+ passedTests.size());
	...
	/**
	 * Removes Passed or Skipped Tests, and any duplicate Failed Tests from Failed
	 * Test List. Duplicates may arise from retries.
	 */
	protected static Set<ITestResult> skippedTestDuplicateRemover(Set<ITestResult> primaryList,
			Set<ITestResult> secondaryList) {
		Collection passedTestListNames = CollectionUtils.collect(primaryList,
				TransformerUtils.invokerTransformer("getMethod"));
		for (ITestResult failedTest : secondaryList) {
			if (passedTestListNames.contains(failedTest.getMethod())) {
				secondaryList.remove(failedTest);
